package com.courierapp.report.controller;
...
public class ReportController {
    ...
    public ResponseEntity<byte[]> exportSummaryExcel(
            ...
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to) {
        return download(reportService.exportExcel(granularity, from, to), XLSX, "booking-summary-" + granularity + ".xlsx");
    }
    ...
    @PreAuthorize("hasAuthority('REPORTS_VIEW')")
    @Operation(summary = "Export booking detail report as PDF")
    ...
    @PreAuthorize("hasAuthority('REPORTS_VIEW')")
    @Operation(summary = "Master party report as PDF")
