...
const EMPTY: FormState = {
  ...
  phone: '', active: true, roleIds: [], departmentId: '', companyId: '',
};
...
export default function UsersPage() {
  ...
  useEffect(() => {
    ...
    adminApi.listActiveDepartments().then(setDepartments).catch(() => undefined);
    adminApi.listActiveCompanies().then(setCompanies).catch(() => undefined);
  }, []);
  ...
  const save = async () => {
    try {
      ...
      const companyId = form.companyId !== '' ? form.companyId : null;
      if (form.id) {
        await adminApi.updateUser(form.id, {
          fullName: form.fullName, email: form.email, phone: form.phone,
          active: form.active, password: form.password || undefined,
          roleIds: form.roleIds, departmentId: deptId, companyId,
        });
        ...
        await adminApi.createUser({
          ...
          fullName: form.fullName, email: form.email, phone: form.phone,
          active: form.active, roleIds: form.roleIds, departmentId: deptId, companyId,
        });
  ...
  const remove = async (id: number) => {
    ...
    try { await adminApi.deleteUser(id); notify('User deleted', 'success'); load(); }
    catch (err) { notify(extractErrorMessage(err), 'error'); }
  };
  ...
  return (
    <Stack spacing={2}>
      ...
      <TextField size="small" label="Search by username or name" value={search}
        onChange={(e) => setSearch(e.target.value)} sx={{ maxWidth: 360 }} />

      <Box sx={{ height: 560, bgcolor: 'background.paper' }}>
        <DataGrid rows={users} columns={columns} loading={loading} disableRowSelectionOnClick
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <TextField label="Username *" value={form.username}
              onChange={(e) => setForm({ ...form, username: e.target.value })} disabled={!!form.id} />
            <TextField label={form.id ? 'New Password (leave blank to keep)' : 'Password *'}
              type="password" value={form.password}
              onChange={(e) => setForm({ ...form, password: e.target.value })} required={!form.id} />
            ...
            <Select multiple value={form.roleIds}
              ...
              renderValue={(selected) =>
                (selected as number[]).length === 0
                  ? <em style={{ color: '#9e9e9e' }}>Select roles…</em>
                  : roles.filter((r) => (selected as number[]).includes(r.id)).map((r) => r.name).join(', ')
              }
            >
              {roles.map((r) => (
